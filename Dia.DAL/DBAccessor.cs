    ...
    public class DBAccessor : IDAContracts
    {
        public IEnumerable<Section> GetSections()
        {
            ...
            using (SqlConnection connection = new SqlConnection(DB.connectionString))
            {
                ...
                command.CommandText = "select id, name from [dia].[sections] ORDER BY name";
                connection.Open();
        ...
        public IEnumerable<User> GetUsers()
        {
            ...
            {
                SqlCommand command = connection.CreateCommand();
                command.CommandText = "select id, name from [dia].[users] ORDER BY name";
                connection.Open();
                SqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
        ...
        {
            IList<Exam> examIds = new List<Exam>();
            using (SqlConnection connection = new SqlConnection(DB.connectionString))
            {
                SqlCommand command = connection.CreateCommand();
                command.CommandText = "select e.id exam_id, s.name section_name, u.name user_name, FilledFrom, FilledTo from dia.exam, e inner join dia.users u on u.id=user_id inner join sections s on s.id=section_id where format(date,'d',filledFrom)=format(date,'d',@report_date) ORDER BY id";
        ...
        public IEnumerable<Meter> GetMeters(int sectionId)
        {
            ...
            {
                SqlCommand command = connection.CreateCommand();
                command.CommandText = "select * from [dia].[getMeters](@section_id) ORDER BY row_number, col_number";
        ...
        public IEnumerable<Meter> GetReadings(long examId)
        {
            ...
            {
                SqlCommand command = connection.CreateCommand();
                command.CommandText = "select * from [dia].[getReadings](@exam_id) ORDER BY  row_number, col_number"; //line_sorter,device_sorter,element_sorter
                ...
                while (reader.Read())
                {
                    try
                    {
                        readings.Add(new Meter()
                        {
                            ColumnNumber = (int)reader["col_number"],
                            RowNumber = (int)reader["row_number"],
                            LineName = (string)reader["line_name"],
                            DeviceName = (string)reader["device_name"],
                            ElementName = (string)reader["element_name"],
        ...
        public long AddExam(Exam exam)
        {
            using (SqlConnection connection = new SqlConnection(DB.connectionString))
            {
                ...
                try
                {
                    ...
                    command.CommandText = "INSERT INTO [dia].[exams]([user_Id],[section_id],[filledFrom],[filledTo])VALUES (@user_Id,@section_id,@filledFrom,@filledTo);\n" +
                    ...
                    if (reader.Read())
                    {
                        long exam_id = (long)reader["exam_id"];
                        transaction.Commit();
                        return exam_id;
                    }
                    else
            ...
            using (SqlConnection connection = new SqlConnection(DB.connectionString))
            {
                ...
                try
                {
                    foreach (Meter reading in readings)
                    {
                        ...
                        command.Connection = connection;
                        command.Transaction = transaction;
                        command.CommandText = "INSERT INTO dia.readings (exam_id, meter_id, value) VALUES (@exam_id, @meter_id, @value)";
        ...
        public void DeleteExam(long examId)
        {
            using (SqlConnection connection = new SqlConnection(DB.connectionString))
            {
                ...
                try
                {
                    SqlCommand command = connection.CreateCommand();
                    command.Connection = connection;
                    command.CommandText = "delete from [dia].[exams] where id=@exam_id)";
                    command.Parameters.AddWithValue("@exam_id", examId);
